...
def show_image_with_axis(title, image, orientation, save_path=None):
    ...
    x_arrow_end = rotate_point(
        (center_x, center_y), (center_x + arrow_length, center_y), orientation_rad
    )
    y_arrow_end = rotate_point(
        (center_x, center_y), (center_x, center_y - arrow_length), orientation_rad
    )

    # Arrow properties
    arrow_props = dict(
        facecolor="white", edgecolor="white", arrowstyle="-|>", linewidth=1
    )

    # Drawing the arrows
    plt.gca().annotate(
        "", xy=x_arrow_end, xytext=(center_x, center_y), arrowprops=arrow_props
    )
    plt.gca().annotate(
        "", xy=y_arrow_end, xytext=(center_x, center_y), arrowprops=arrow_props
    )
    ...
    if save_path:
        plt.savefig(
            save_path, bbox_inches="tight", dpi=300, pad_inches=0
        )  # bbox_inches='tight' removes excess whitespace

    plt.show()


def image_to_contours(img_path):
    # Load the image
    ...
    return img_line_segments, orientation


def main():
    img_line_segments, orientation = image_to_contours(
        "dev/image-manipulation/data/cal.png"
    )

    output_dir = Path(os.getcwd(), "output")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = Path(output_dir, f"contoured_logo_line_segments_{orientation}.png")
    cv2.imwrite(output_file.as_posix(), img_line_segments)

    show_image_with_axis("Test", img_line_segments, orientation, output_file.as_posix())
